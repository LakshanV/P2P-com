...
test('creates, places, confirms and completes end-to-end against the real schema', liveTestOptions, async () => {
...
test('the database refuses a line whose total is not quantity times unit price', liveTestOptions, async () => {
...
test('the database refuses a status that disagrees with its timestamps', liveTestOptions, async () => {
  await withTestDatabase(async ({ database, directory }) => {
    await migrateUp(database, { directory });
    ...
    assert.match(cancelledWithout, /cancelled_at_matches_status|cancellation_reason_matches_status/);
...
test('the database refuses to rewrite or delete a line, a snapshot or a transition', liveTestOptions, async () => {
  await withTestDatabase(async ({ database, directory }) => {
    await migrateUp(database, { directory });
    const service = new OrderService(new PostgresOrderRepository(database));
...
test('a cancelled order keeps its reason, and the transition log survives', liveTestOptions, async () => {
  await withTestDatabase(async ({ database, directory }) => {
    await migrateUp(database, { directory });
    const service = new OrderService(new PostgresOrderRepository(database));
    const { orderId } = await fullLifecycle(service, 'cancel1');

    await service.cancelOrder(cancelRequest(orderId, { cancellationReason: 'stock-unavailable' }));
...
test('a total above Number.MAX_SAFE_INTEGER survives the round trip exactly', liveTestOptions, async () => {
  await withTestDatabase(async ({ database, directory }) => {
    await migrateUp(database, { directory });
    const service = new OrderService(new PostgresOrderRepository(database));
