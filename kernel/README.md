...
**Fourteen components have implemented foundations.** K-05 and K-08 are build step B-1; K-01, K-02,
K-03, K-04, K-07 and K-09 are B-2, which is covered; K-06 Policy Engine, K-10 Ledger Foundation, K-12 Conversation Foundation, K-13 AI Gateway, K-14 Notifications and K-15 Search Foundation are the first of **B-3**. The kernel's internal chain is `K-01 → K-02/K-03 → K-04`, and **every
...
| **K-06 Policy Engine** | `policy-engine/` | [`policy-engine/CONTRACT.md`](./policy-engine/CONTRACT.md) | Versioned business policy, and **every successful evaluation returns the version id that produced it** — which is what makes v3 §35's "historic transactions retain the policy version originally applied" keepable. An explicit draft → publish → activate → retire lifecycle, immutable numbered versions, bounded effective windows, a scope hierarchy whose precedence is decided by specificity with **ties refused rather than resolved by row order**, six literal predicate shapes that cannot compute, and typed outputs. Every rate, threshold and amount is an exact `{ units, scale }` decimal compared through `BigInt`: **no floating point exists anywhere in the component**, because money computed from an inexact rate is money nobody can reconcile. Reads K-05 only for a declared `configured` output, pinning the configuration version beside the policy version. **Nothing evaluates a policy**, no amount has been priced by one, and no audit record or event follows a published rate. Delivered by FND-005b. |
| **K-10 Ledger Foundation** | `ledger-foundation/` | [`ledger-foundation/CONTRACT.md`](./ledger-foundation/CONTRACT.md) | Double-entry ledger primitives: an asset-type registry, accounts, balanced transactions, entries and derived balances. Every amount is an exact integer in minor units; no floating point exists anywhere in the component. A deferred constraint trigger refuses unbalanced, negative, mixed-asset-type or unknown-account transactions at commit, and append-only triggers refuse any update or delete of ledger history. **Nothing calls it**, so no value has moved, and no audit record or event follows a posted transaction. Delivered by FND-005c. |
| **K-12 Conversation Foundation** | `conversation-foundation/` | [`conversation-foundation/CONTRACT.md`](./conversation-foundation/CONTRACT.md) | Conversation primitives: a container with context and title, participants with roles, and messages with content and type. Every identifier and instant is caller-supplied, so the component reads no clock and generates no randomness. Append-only triggers refuse any update or delete of conversation history, and duplicate account participation in one conversation is refused. **Nothing calls it**, so no conversation, participant or message is created by any unit. Delivered by FND-005d. |
| **K-13 AI Gateway** | `ai-gateway/` | [`ai-gateway/CONTRACT.md`](./ai-gateway/CONTRACT.md) | The single runtime boundary to model providers: task definitions, model bindings, deterministic routing, mock-provider execution, cost capture, AI decision recording, idempotent creation, append-only records, caller-supplied identifiers and instants, and a module-owned outbox. Foreign fields are refused by name so no business, profile or financial data is smuggled into a gateway record. **No live provider adapter ships**, so no task is executed by a real model and nothing calls it. Delivered by FND-005e. |
| **K-14 Notifications** | `notifications/` | [`notifications/CONTRACT.md`](./notifications/CONTRACT.md) | Channel-neutral notification primitives: channel configurations, notifications and append-only delivery attempts. The service resolves a configured provider per channel, enforces idempotent creation, stores caller-supplied identifiers and instants, and emits K-08 events and K-09 audit records through the module outbox. **Only the in-app provider ships**, so no real email, SMS, push or WhatsApp message is sent, and no scheduling worker or template service exists. Delivered by FND-005g. |
| **K-15 Search Foundation** | `search-foundation/` | [`search-foundation/CONTRACT.md`](./search-foundation/CONTRACT.md) | Search document and query-log primitives: keyword full-text search with a generated `tsv` tsvector, idempotent indexing and query logging, foreign-field refusal, append-only query log, deterministic caller-supplied identifiers and instants, and migration 0021 under `kernel_search_foundation`. `index`, `query` and `remove` each emit one K-08 event and one K-09 audit record through the module-owned outbox. **No caller indexes or searches anything**, and nothing calls it; no API, no UI, no semantic/vector search, and nothing applied to a live server. Delivered by FND-005h. |
...
| Deferred | For all fourteen | Where it bites hardest |
|---|---|---|
...
| Audit trail | There is nowhere the components themselves write who did what. **K-09** exists and none of the other thirteen uses it | Creating an identity, opening an account and signing in are exactly the sort of actions K-09 was built to record, and none is |
| Real usage | **No module publishes or consumes an event, no unit records an audit record, no unit creates an identity subject or opens an account, no caller authenticates, no caller indexes or searches anything, and nothing asks K-04 whether it may act.** The event types, audit actions, verifiers and policies in the tests are fixtures | All eleven transaction-enlisted paths (K-01, K-02, K-03, K-04, K-06, K-07, K-08, K-09, K-10, K-14 and K-15) exist as capabilities rather than integrations. K-02 → K-01, K-03 → K-01 and K-04 → K-02/K-03 are the exceptions that prove the point: real cross-component dependencies, exercised by tests, with no *caller* on any |
...
**One component remains unbuilt** — K-11 Commerce Unit Registry — and it is build step B-3.
B-1 and B-2 are both covered, and K-06, K-10, K-12, K-13, K-14 and K-15 are the first of B-3. Its
