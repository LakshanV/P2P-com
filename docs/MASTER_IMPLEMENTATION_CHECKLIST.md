...
**474 tracked items** carry an explicit status. Counts verified mechanically over this file after FND-004c, which moved the K-02 contract to COMPLETE and the K-02 implementation, P0-33, X-01, X-02 and X-14 to IN PROGRESS. The method is reproducible and is asserted by `tests/status-docs.test.ts`: count one status marker per status-bearing cell of every row in sections A–G and J whose first cell is an item id. §H and §I are registers rather than tracked requirements and are excluded, which is what makes the section table below sum to the same 474.

| Status | Count | Notes |
|---|---:|---|
| `[ ]` NOT STARTED | 421 | |
| `[~]` IN PROGRESS | 28 | X-52 (standing documentation obligation); P0-04…P0-08 (satisfied by FND-001a, held while FND-001 is unfinished); P0-10 (four of eight MODULE_MAP §13 checks built); X-44 and X-46 (the checks exist but are not yet **CI** checks); P0-14, P0-15, P0-16 and RC-06 (FND-002a delivered the migration contract, FND-002b the runner, FND-002c local provisioning); P0-17 (seed and fixture foundation, never loaded); T-21 (test-database lifecycle delivered, never run); P0-35 and P0-36 (K-08 and K-09 cores); P0-38 (K-05's core; K-06 not started); **P0-33, X-01, X-02 and X-14 (K-02's core, with no verifier behind it and no HTTP surface in front of it)**; and the K-01, K-02, K-03, K-05, K-08 and K-09 implementation rows. Nothing has been executed against a live PostgreSQL. |
| `[?]` NEEDS REVIEW | 0 | Nothing has been submitted for review. |
| `[x]` COMPLETE | 11 | P0-01 (DOC-001 documentation artefacts), P0-03 (all six source roots tracked and documented), P0-11 (financial-zone and provider-import checks, proven by planted fixtures), P0-12 and P0-13 (contributor documentation and git conventions), and six kernel module contracts — K-01, K-02, K-03, K-05, K-08 and K-09. A contract is a fixed public surface, not a delivered capability. |
...
**Business capabilities complete: 0.** No kernel component, no business module, no phase, no release gate. Six kernel components have *contracts* marked COMPLETE and *implementations* marked IN PROGRESS; a contract is a fixed public surface, not a delivered capability, and §B distinguishes the two columns for exactly this reason. Phase 0 itself is not complete: the data layer (§A.2) has never run against a live server, environments (§A.3) do not exist, platform conventions (§A.4) are partial, and of the foundation capabilities (§A.5) only authentication, events, audit and configuration have moved off NOT STARTED — identity and accounts are delivered but have no rows there at all, and are tracked in §B instead.
