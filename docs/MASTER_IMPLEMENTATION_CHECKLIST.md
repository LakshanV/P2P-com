...
Each kernel component needs a contract document (v3 §10) and an implementation meeting the v3 §55 Definition of Done. Both are tracked. Seven components — K-01, K-02, K-03, K-04, K-05, K-08 and K-09 — have a `COMPLETE` contract and an `IN PROGRESS` implementation; **none is complete**, and the remaining eight are `NOT STARTED`.
...
**474 tracked items** carry an explicit status. Counts verified mechanically over this file after FND-004d, which moved the K-04 contract to COMPLETE and the K-04 implementation, P0-34, X-03, X-04 and X-20 to IN PROGRESS. FND-004d's three corrections (§11.27–§11.29) changed no status and added no row, so the counts below are unchanged by them. The method is reproducible and is asserted by `tests/status-docs.test.ts`: count one status marker per status-bearing cell of every row in sections A–G and J whose first cell is an item id. §H and §I are registers rather than tracked requirements and are excluded, which is what makes the section table below sum to the same 474.
