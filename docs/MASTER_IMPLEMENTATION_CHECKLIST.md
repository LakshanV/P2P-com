...
| M-09 | RFQ / Reverse Marketplace | L4 | `[x]` COMPLETE — [`modules/rfq/CONTRACT.md`](../modules/rfq/CONTRACT.md) | `[~]` IN PROGRESS — asking the market, once every other way has been tried. A tender exists **because M-07's sourcing ladder could not solve the Need**, and `matchRunId` records the run that reached that conclusion — an RFQ nobody can justify is one a supplier is entitled to resent. The module's central rule is that **the customer's words never reach a supplier**: a Need is a sentence somebody wrote, deliberately exempt from the identifier rules and liable to hold a telephone number or a hint about what the buyer will pay, and a tender goes to strangers. Defended four ways — a `CARRIED_KEYS` allowlist so a reading's unrecognised keys do not travel; a private-text guard that refuses anything shaped like a pasted message; a **shape with no free-text field wide enough to hide a Need in**, `item_description` capped at 500 characters because a field long enough to hold a customer message will eventually hold one; and a **schema of named, typed columns rather than a `jsonb` blob**, asserted as an exact column set so adding a `notes` column fails a test rather than passing for want of a pattern nobody thought of. **No specification travels in an event** either: a private tender whose contents sit in a log every subscriber reads is not private, so an invited supplier fetches the requirement through a route that can check the invitation. `UNIQUE (rfq_id, supplier_account_id)` makes inviting somebody twice impossible; the service converges on the existing invitation, keeps the **original** reason and rung, and publishes exactly one notification. Invitations and transitions are append-only by trigger — they have already seen it. An award names exactly one winner in both directions by CHECK. One defect was found by writing the suites: **awarding an already-awarded tender to a different quote returned `replayed`**, so a second decision was reported as a successful retry while changing nothing; it now throws `illegal-transition`, and re-awarding the same quote still converges. **Not complete: no HTTP route, no UI, no consumer of any of the five events, no K-14 notification actually sent, no PostgreSQL-backed caller.** Unit tests plus 8 live-PostgreSQL integration tests, migration 0052. | B-9 |
| M-10 | Quotes | L4 | `[x]` COMPLETE — [`modules/quotes/CONTRACT.md`](../modules/quotes/CONTRACT.md) | `[~]` IN PROGRESS — what the market offered back, and the comparison a customer decides on. **An offer binds, so it cannot be edited**: a supplier who quoted 250,000 has said they will supply for 250,000, and a market where that is quietly revised is one where the offer you accepted is not the offer you saw. Changing a price means withdrawing and submitting a new one, which leaves both on the record. Enforced at three layers — the service has no such operation, the adapter's UPDATE sets four columns, and `quote_terms_are_immutable` refuses any UPDATE touching a term, refuses DELETE, and refuses to move an offer that has already ended. **A supplier acts only on their own offer**, checked in the service rather than only at the edge, because withdrawing a competitor's offer would otherwise be the cheapest way to win a tender; and **only an invited supplier may quote**, checked against M-09 through a two-method port rather than read from the request. **Ranking is not price**: five weighted factors as integers per mille — cost 350, reliability 200, lead time 200, completeness 150, quality 100 — with the weights as data a buyer can override, a supplier with no delivery record scored 600 rather than 0 so the market stays open to new entrants, unavailable offers shown with their reason rather than dropped, an explanation on every score, and **exactly one recommendation the customer may always override**, because one they could not would be a decision taken from them. `partial` and `substitute` are first-class — three partials make a split no single supplier could fill — and a CHECK ties `substitute` to a declared difference in both directions. **No price travels in an event** (the log is read by every subscriber and kept indefinitely) while the **audit record does** carry it, because it answers what was actually agreed. No score, rank or recommendation is stored anywhere. Two defects were found by writing the suites: the ranker took one reliability figure for a whole set, so every supplier would have been scored against the first one's record; and it compared instants with `Date.parse`, which answers NaN for anything it does not recognise — and a comparison against NaN is silently false, which here would have meant an expired offer presented as live. **Not complete: no HTTP route, no UI, no M-11 consumer of `quote.accepted`, no order opened from an accepted offer, no supplier reliability actually sourced from delivery history.** 38 unit tests, 9 live-PostgreSQL integration tests, migration 0053. | B-9 |
| M-11 | Orders | L5 | `[x]` COMPLETE — [`modules/orders/CONTRACT.md`](../modules/orders/CONTRACT.md) | `[~]` IN PROGRESS — the order state machine, the immutable commercial snapshot, and split supplier fulfilment. An order is an agreement, so an item pins `(listingId, versionId)` — the permanent address M-04 exists to provide — and the `OrderSnapshot` captures the agreed terms at placement; both tables are append-only by trigger, because an agreement that changes after the fact is not a record of what was agreed. The state machine is a declared table, not scattered conditionals, and cancellation is reachable from every non-terminal state since an order you cannot abandon is a trap. **Split fulfilment**: a 20-tonne order becomes a parent with one child order per supplier, `allocation-mismatch` refuses any allocation that does not sum to the ordered quantity for every pinned version, and **partial fulfilment is a derived quantity, not a status** — `getFulfilmentSummary` sums child rows, so nothing stores a ratio that drifts when a child moves. A parent completes only when every child is terminal, refuses `nothing-fulfilled` when all cancelled, and cascades cancellation to non-terminal children only. Money is `bigint` minor units with the arithmetic checked by the database as well as the service (`order_item_line_total_is_product`); no floating-point column exists. M-12/M-13/M-14 are the same layer and are refused **by name**, reaching Orders by event only; a regression asserts no source file mentions them or the AI gateway. Three defects were found by running the suites: concurrent splits both succeeded (neither repository had optimistic locking, so the same 20 tonnes could be promised twice — fixed with a conditional `UPDATE` and a commit-time guard); `order.cancelled` published only free text where a consumer needs a vocabulary word; and a split parent reaching `fulfilling` without ever being `confirmed` violated a CHECK the in-memory repository does not enforce, so **all 42 unit tests passed while every split failed against a real database**. **Not complete: nothing calls it** — no API, no UI, no consumer of any of the seven events, no M-04 inventory reservation actually taken, no K-02 authentication, no K-04 authorisation, no payment linkage (M-12 is unbuilt). An order line now names **either** a listing version **or** an accepted M-10 quote, exactly one of the two in both directions by CHECK (migration 0054): a tender exists because no listing answered, so there is no version to pin, and faking one would put a row in M-04 that nobody published and no supplier maintains. `line_kind` separates `goods` from `charges`, because a quote's landed total is deliberately not `quantity * unit_price` -- the difference is delivery, duties and handling -- and the line arithmetic is a database rule that must keep holding; so an accepted offer opens a goods line at the exact product plus a charges line for the rest, and the buyer sees what they are being charged for rather than a unit price quietly inflated to absorb delivery. **The tendering path now reaches an order**: `apps/api/consumers/quote-order.ts` opens, places and confirms one from `quote.accepted`, reading the terms from M-10 (no price travels in the event) and the buyer from M-09 (never from anything a caller can influence). Confirmed rather than left at placed, because a quote binds and asking the supplier to confirm again would give them a second chance to decline what they were bound to. 42 unit tests, 11 consumer tests, 14 live-PostgreSQL integration tests, migrations 0028, 0029 and 0054. | B-10 |
...
| M-13 | Financial Ledger | L5 | `[x]` COMPLETE — [`modules/financial-ledger/CONTRACT.md`](../modules/financial-ledger/CONTRACT.md) | `[~]` IN PROGRESS — the wallet map and the value router over K-10. A **wallet** names a K-10 account: this party, this asset type, **this purpose**. Earnings is a purpose and not an asset class, which matters because a seller’s earnings and a buyer’s spending money may both be rupees, and a separate class would produce a second kind of rupee that cannot be added to the first. A **value plan** pays one obligation from several kinds of value at once: LKR 10,000 as 1,500 reward points + 500 merchant credit + 8,000 on a card is three balanced transactions in three units, because K-10 refuses — correctly — to put lines denominated in two units in one transaction. The plan’s invariant is that the legs’ settlement equivalents sum to the obligation **exactly**: under-covering is a short payment nobody noticed, over-covering takes value for nothing. Rates are integer pairs checked by cross-multiplication, so **nothing rounds** — an allocation that does not divide evenly is refused rather than losing a fraction of a cent per leg. **No balance is stored anywhere in the module**; K-10 derives every one, and a column here would be a second source of truth about money. Postings go to the journal **before** the leg is recorded, so a crash leaves money moved and a leg still `planned` — which a retry under the same key converges — rather than a leg claiming a posting that never happened. A cancelled plan is **reversed by compensating transactions**, never deleted, and `settled` is deliberately not terminal because an order paid for in full still gets cancelled. Two defects were found by running the suites: `settled` had been made terminal, which would have put the only route back for a delivered-then-returned order outside the ledger entirely; and K-10 refused `precision = 0`, so an indivisible asset could not be registered at all (fixed by migration 0031, with K-10’s validator, test and contract updated). **Not complete: nothing calls it** — no API, no UI, no consumer of M-12’s `payment.captured` feeding the external leg, no relay dispatching the outbox, and no reconciliation job for the gap between K-10’s transaction and M-13’s record. 39 unit tests against a real K-10 journal, migration 0031. | B-10 |
...
| M-36 | User Cockpit | L8 | `[x]` COMPLETE — [`modules/user-cockpit/CONTRACT.md`](../modules/user-cockpit/CONTRACT.md) | `[~]` IN PROGRESS — MY MONEY, MY ORDERS and one order in detail, assembled from K-10, M-11, M-12 and M-13. **The module stores nothing** — no repository, no migration, no cached total — and a test asserts it: a cockpit with its own store is a second source of truth about money, and the second source is the one that goes stale, which on a balance screen is not a performance characteristic but a lie with a timestamp. The module therefore cannot be wrong about money, only slow. **MY MONEY never shows one total.** It is a list of holdings per asset type, summed only within one, because 1,500 reward points plus LKR 8,000 is not 9,500 of anything: points have an issuer, an expiry and a list of things they may be spent on, and rupees have none of those. Each position carries `withdrawable`, `transferable`, `issuer` and `precision` beside the number, because a holder looking at a reward balance needs to know it is not cash and next to the number is the honest place to say so — which is what K-10 records those attributes for. The three positions (available, pending, locked) are reported separately, and `empty` distinguishes “you have no wallets” from “every balance is zero”. A wallet naming a K-10 account that does not exist is **refused**, not shown as zero: inventing a balance is worse than failing. Delivering it required adding `findAssetType` to K-10, which recorded an asset’s issuer, withdrawability and expiry and provided no way to read any of it back. **Not complete: nothing asks K-04 whether the caller may read this account**, so the only thing between one holder and another’s balance is that no client has been written — the module’s most important gap. No paging, no restriction enforcement, no UI. 15 unit tests. | B-5 shell |
...
| M-48 | Supplier & Merchant Directory | L2 | `[x]` COMPLETE — [`modules/supplier-directory/CONTRACT.md`](../modules/supplier-directory/CONTRACT.md) | `[~]` IN PROGRESS — the network the sourcing ladder searches, and the module M-07 was built against and did not have. Until it existed the `known` and `verified` rungs returned `unavailable` on every Need, so the ladder consulted the catalogue and then escalated: **every Need that no published listing answered became a tender**, which is the behaviour the ladder exists to prevent. Four properties carry it. **Registration is not activation**: a new entry is `pending` with `acceptsOrders: false` and the caller cannot ask for anything else, because a platform where signing up put you into the market would give the first tender to whoever registered fastest. **Category is a gate**: `findSuppliers` refuses a query with no category (`ungated-query`) rather than answering with everybody, because an ungated directory query is the platform's commercial map — who trades here, how many, and how to reach each one — leaving through an endpoint built for sourcing; and districts filter permissively, since a supplier who declared none said "no restriction stated", not "serves nowhere". **Claims are separated from facts**: whether a party is verified is M-02's and what they actually delivered is M-11's, and `FOREIGN_FIELDS` refuses both by name, because a directory holding a stale copy of either is the second answer somebody sources against. **A declaration does not travel**: the four events carry the entry, the account, the kind and the status, and never the categories, brands or districts — the event log is read by every subscriber and kept indefinitely, and a directory that broadcast its own contents would be a market-intelligence feed nobody agreed to publish. Facet values are held to a **code** rule rather than the opaque-identifier rule, because a shared vocabulary is meant to be enumerable — a buyer picks `cement` from a list — while a value with no letters or a long run of digits is a telephone number and would travel into every invitation the supplier received; the same rule exists in the database as `is_facet_code`. Three database rules the service cannot express: `UNIQUE (account_id)` (a party with two rows can be invited twice to one tender and answer once), `UNIQUE (supplier_id, facet_kind, value)`, and a **partial** unique index giving one primary branch among the open ones, so closing a head office frees the slot rather than trapping the supplier. One defect was found by writing the cycle test: the outbox id was keyed `supplierId:status`, so a suspend-then-reactivate collided on `supplier.activated` and the platform would have silently stopped publishing reinstatements; it is now keyed on the caller's transition id. **The two empty sourcing rungs now read it.** `apps/api/supplier-source.ts` is the join M-07 could not make itself: claims from M-48, verification from M-02, prior trade from M-11 — three modules that may not import one another, joined where the application deploys all three. `known` starts from orders this buyer actually **completed**, because a placed order is a promise and counting it would make the strongest signal on the platform the easiest to fake; `verified` starts from the gated directory query and keeps only what M-02 verified. A failure propagates rather than returning empty, so M-07 records `lookup-failed` instead of quietly escalating every Need the week the database is slow. **And it is reachable**: `apps/api/routes/directory.ts` serves fourteen routes where a party registers under their own session — never an `accountId` in the body — declares categories, brands, capabilities and districts, opens branches, sets availability, and is admitted by somebody else. Admission is the K-04 action `admit`, held by no trading role, on a route marked `actsOnAnotherParty`: the ownership check is skipped because it would refuse the act the route exists for, the caller must declare a purpose K-04 checks against the grant and records on the decision, and the handler still refuses an operator deciding their own entry. **Not complete: no UI, no notification when a party is admitted or suspended, no merchant listing linkage, and no supplier reliability figure — M-07 scores every party as having no delivery record, which is honest and is not the same as having one.** 25 unit tests, 20 route tests, 10 adapter tests, 12 live-PostgreSQL tests, migration 0057. | B-7 |
| M-49 | Organisations | L1 | `[x]` COMPLETE — [`modules/organisations/CONTRACT.md`](../modules/organisations/CONTRACT.md) | `[~]` IN PROGRESS — the business, and the people who act for it. Every commercial record in this platform names an **account**, and until now that account was always a person's: true of a sole trader, false of every business with two people in it. The decision that makes the rest cheap is that **an organisation is a K-03 account of its own** — so a listing, an order, a wallet and a directory entry belong to the business with *no change to any of their modules*, and a business that adds staff or changes owners rewrites nothing (D-053). A **membership** says one person may act for one organisation in named roles, and it is scoped by construction: a FINANCE membership at one business is a different row from anything at another, so it confers nothing there. Six rules, enforced in the module and — where they span rows — in the database. Creation is **atomic with the owner**, so there is no window in which a business exists that nobody can administer. Creating is not being admitted: a new organisation is `pending`, and having ten employees does not make it sourceable. Joining is agreed to: an invitation confers nothing until the invited person accepts, and only they can. **Nobody confers what they do not hold** — OWNER and ADMIN build a team, only an OWNER may make, demote or remove another OWNER. Nobody decides their own place, except by *leaving*, which is a separate call because needing permission to stop working somewhere is not a thing a platform should impose. And **an organisation always has an active owner**: refused four ways in the service and by `membership_keeps_an_owner`, a **deferred** constraint trigger — deferred precisely so founding a business, which writes two rows in one transaction, is not refused halfway through. A hole the tests found: `changeRoles` let an ADMIN demote an OWNER while `revokeMembership` refused it — demotion is the quiet takeover, since nothing about "change their roles" reads as removing anybody. K-04 gains the `ORG_*` role namespace (migration 0059) and, in D-054, evaluates authority in an account the subject holds a grant in rather than only their own — which is what makes acting-for possible without impersonation. The API declares it with `x-acting-for`, never infers it, and reads the membership live so a suspended member stops on their next request. **Not complete: no HTTP routes yet, no staff-invitation UI, no transfer-of-ownership operation (done today by making a second owner and leaving), and organisations are not yet what the supplier directory registers.** 27 unit tests, 9 live-PostgreSQL tests, migrations 0058 and 0059. | B-2 |
