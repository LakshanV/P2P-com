...
| HTTP API | **Foundation only** — `apps/api`. 37 routes over M-11, M-12, M-13 and M-36 on `node:http`, with RFC 9457 problem responses that keep each module’s own refusal code, an `Idempotency-Key` requirement on every write, correlation ids on every response, and a request context that is the one place a clock is read or an identifier generated. **No authentication and no authorisation**: K-02 ships no verifier and nothing asks K-04, so every route is open to anyone who can reach the port. Not deployed | apps/api |
