...
| Listings | **Foundation only** — the listing half of M-04. A listing is a stable identity whose terms change by **version, never by edit**, so an order that pinned version 3 still reads what version 3 said; prices are `bigint` minor units with no floating-point column in the schema; media is an opaque handle, never the artefact. **The inventory interface is not built** — `getAvailability`, `reserve`, `release`, `commit`, `receive` and `adjust` do not exist, and that is the module's replaceability requirement | M-04 |
