...
| Migrations | 59 forward + 59 rollback, validated statically by `npm run check:migrations`, applied by `npm run db:migrate` (FND-002b). Migrations 0013–0016 added module-owned transactional outbox tables for K-05 (`kernel_configuration.outbox`), K-06 (`kernel_policy_engine.outbox`), K-07 (`kernel_feature_flags.outbox`) and K-11 (`kernel_commerce_unit_registry.outbox`), so each module can append domain events and audit records atomically with its own business mutation. Migration 0018 adds the `kernel_conversation_foundation` schema with K-12's conversation, participant, message and outbox tables. Migration 0017 adds the `kernel_ledger_foundation` schema with K-10's asset-type registry, ledger accounts, balanced transactions, entries and outbox. Migration 0019 adds the `kernel_ai_gateway` schema with K-13's task-definition, model-binding, AI-run, AI-decision and outbox tables. Migration 0020 adds the `kernel_notifications` schema with K-14's channel, notification, delivery-attempt and outbox tables. Migration 0021 adds the `kernel_search_foundation` schema with K-15's document, query-log and outbox tables. Migration 0022 extends K-10 with the universal multi-value model: eight asset-type attributes (`issuer`, `unit`, `redeemable`, `convertible`, `expiry_days`, `restrictions`, `custody_provider`, `jurisdiction`) and a `balance_state` column on `ledger_entry` that splits every account into three derived positions — available, pending and locked — with the entry primary key widened to match, so a transaction may move value between two positions of one account without changing its total. Migration 0023 adds K-13's `task_authority` table — an append-only version history of what one task is permitted to do, carrying a 0-4 ceiling, a suspension flag that is the kill switch, and a required rationale — plus an `authority_level` column on `ai_run` recording the level each run actually executed under. `executeTask` now refuses a task with no grant in force, a suspended task at every level including observe, and any request above the granted ceiling. They create the `platform` schema, the migration ledger, the `kernel_configuration` schema with K-05's version and outbox tables, the `kernel_event_infrastructure` schema with K-08's event log, delivery and receipt tables, and the `kernel_audit_foundation` schema with K-09's append-only audit table, the `kernel_identity` schema with K-01's write-once subject table, the `kernel_accounts` schema with K-03's universal-account table and its `UNIQUE (subject_id)` one-account-per-party constraint, the `kernel_authentication` schema with K-02's binding, evidence and session tables, the `kernel_permissions` schema with K-04's policy-version, grant, revocation and decision tables, the `kernel_feature_flags` schema with K-07's flag-version, activation, lifecycle and outbox tables, the `kernel_policy_engine` schema with K-06's draft, version, activation, retirement and outbox tables, the `kernel_commerce_unit_registry` schema with K-11's type-version, activation, retirement and outbox tables, the `kernel_ledger_foundation` schema with K-10's asset-type, account, transaction, entry and outbox tables, and the `kernel_conversation_foundation` schema with K-12's conversation, participant, message and outbox tables, and the `kernel_ai_gateway` schema with K-13's task-definition, model-binding, AI-run, AI-decision and outbox tables. Migration 0027 extends M-04 with the inventory interface: `inventory_movement` (append-only) and `inventory_snapshot` (derived per listing version) under `module_universal_listing`, with CHECK constraints `reserved <= on_hand`, `on_hand >= 0`, `reserved >= 0` and `committed >= 0`, and a trigger refusing mutation of `inventory_movement`. Sixty-two triggers across sixty-two tables refuse to update or delete a row — M-10's `quote_terms_are_immutable`, which permits a status change away from `submitted` and refuses every change to a price, a quantity, a lead time or a validity, because an offer binds; all four of K-04's, because authority history is append-only; K-13's `task_authority` trigger, because a grant is a new version and never an edit; K-12's three, because conversation records are append-only; K-02's session trigger permits exactly two changes — rotate the secret, record a revocation — and refuses everything else; K-14's two, because channel and delivery-attempt records are append-only; M-01's `capability_state` trigger, because the log of how a capability reached its current status is append-only; M-11's three, because an order's lines, its commercial snapshot and its transition log are what a dispute is judged against; M-12's three, of which two refuse any change to a provider attempt or a refund and the third permits exactly one — stamping a webhook receipt processed — because re-stamping it would erase when the platform actually acted on a provider's instruction; M-13's, because how a wallet came to be frozen is not editable; and M-48's `directory_event_is_append_only`, because the sequence by which a supplier was registered, activated, suspended and reinstated is what an appeal is judged against, and a directory that could rewrite it could deny a suspension ever happened. Migration 0024 creates the first business-module schema, `module_universal_account`, with M-01's `account_capability` table (one row per account and capability, `UNIQUE (account_id, capability)`), its append-only `capability_state` transition log and its outbox. Its `account_id` is an opaque K-03 identifier and deliberately not a foreign key: Migration 0025 creates the second business-module schema, `module_capability_verification`, with M-02's verification-case table, its append-only `evidence` and `level_record` tables and its outbox. A partial unique index allows one *open* case per (account, purpose) while leaving a decided or withdrawn case unable to block the next attempt, Migration 0026 creates the third business-module schema, `module_universal_listing`, with M-04's `listing` table and its three append-only companions — `listing_version`, `listing_media` and `listing_declaration`. A listing is versioned and never edited, so an order placed against version 3 still reads what version 3 said; prices are `bigint` minor units with no floating-point column anywhere in the schema; and `listing_media.reference` obeys the same opacity rule M-02 applies to evidence, for the same reason. Migration 0028 creates the fourth business-module schema, `module_orders`, with M-11's `order_header` — named so because ORDER is a reserved word in SQL — and its three append-only companions `order_item`, `order_snapshot` and `order_event`. `order_item_line_total_is_product` makes the line arithmetic a database rule, and `UNIQUE (order_id)` on `order_snapshot` stops an order having two agreements with no way to say which one a dispute is judged against. Migration 0029 extends M-11 with split-supplier fulfilment: `parent_order_id`, `fulfilment_role`, the CHECK that a child has a parent and a parent does not, and a relaxation of 0028's confirmation rule for parents, which reach `fulfilling` by being split rather than by being confirmed. Migration 0030 creates the fifth, `module_payments`, with M-12's `payment` header, the append-only `payment_attempt` reconciliation trail, `refund`, and `webhook_receipt` whose `UNIQUE (provider, provider_event_id)` is what makes the redelivery every provider eventually sends take effect exactly once. Its `asset_code` is open to any settlement asset an external counterparty can actually settle — LKR, USD, BTC, USDC — and closed by name to the value JAYA issues itself, because no bank, card network or custodian has heard of a reward or a merchant credit and a row claiming one settled externally is a claim on money the platform never held. `payment_captured_within_authorised` and `payment_refunded_within_captured` are the two invariants that keep a payment honest, and `payment_instrument_token_opaque` is what stops a card number being stored as a "token". Migration 0031 widens K-10’s `asset_type` precision constraint from `> 0` to `0..18`, because 0017 had quietly made an **indivisible** asset unrepresentable — a loyalty stamp, a ticket, a whole-unit reward point — and the rule disagreed with M-12, which already accepted a scale of zero. It is a widening, so no existing row can be invalidated; its rollback can fail by design, because narrowing it again with indivisible asset types on record is a decision about money that a migration must not make on an operator’s behalf. Migration 0032 creates the sixth business-module schema, `module_financial_ledger`, with M-13’s `wallet` (one per owner, asset type and purpose, and one per K-10 account), its append-only `wallet_state` history, `value_plan` and `value_leg`. Two of its rules are worth naming. `value_leg_rate_is_exact` makes the no-rounding rule a database rule, written as a multiplication rather than a division so a rate that does not divide evenly cannot be stored at all. `value_plan_legs_sum_to_target` is a **deferred constraint trigger**, because the invariant spans rows and no CHECK can express it: it fires at commit and refuses any plan whose legs do not add up to what is owed, so the last thing between a short payment and the ledger is not code somebody might edit. A partial unique index gives one live plan per obligation while leaving a cancelled attempt out of the way. **No balance column exists in the schema, and none ever may**: K-10 derives every balance, and a column here would be the second source of truth that is wrong. Migrations 0033–0047 give every one of the fifteen outbox tables what a production relay needs: `next_attempt_at`, so a failed row is rescheduled rather than retried on every poll; `dead_lettered_at` and `dead_letter_reason`, so a row that will never succeed is given up on with the reason recorded; a CHECK that the two agree, and another that a dead-lettered row is never marked processed — it was not dispatched, and saying otherwise would tell every reader the opposite of what happened; and a partial `outbox_claimable_idx` covering only what is still to do, so the index stays the size of the backlog rather than the size of history. The change lands once per owner rather than as one migration reaching across fifteen namespaces, because `check:migrations` refuses the latter and is right to: the outbox is a platform contract, but each unit owns its own table. Migration 0048 adds `listing_version.inventory_mode` — how a version's fulfilment relates to stock, as one of `tracked`, `untracked`, `external`, `made-to-order` or `digital`. A policy rather than a boolean, because a supplier-direct machine, a made-to-order part and a digital entitlement are not "untracked" in the way a haircut is, and collapsing them would force the whole order and inventory path to be migrated the first time one arrived. It sits on the **version** rather than the listing, because changing it changes the terms and an order pins a version precisely so agreed terms stay what they were. Existing rows backfill to `tracked`, which is what every version behaved as when the reservation path was the only one there was; the default is then dropped, so publishing a version means deciding this. Migration 0049 creates the seventh business-module schema, `module_commerce_request`, with M-03's `request` and its three append-only companions `request_interpretation`, `request_media` and `request_event`. Its defining rule is a trigger refusing any UPDATE that would change `request.raw_text`: what somebody asked for is evidence, and a correction is a new row in `request_interpretation` rather than an edit to the words. `raw_text` is the one column in the platform deliberately outside `is_opaque_identifier` — it is a sentence a person wrote, not a key — and the cost of that is recorded in the migration itself: M-03 publishes the text's *length* in its events and never its content. Confidence is an integer per-mille from 0 to 1000, because there is no floating-point column anywhere in this schema either. Migration 0050 creates the eighth, `module_matching`, with M-07's `match_run`, `rung_attempt` and `match_candidate`. All three are append-only, because re-running the sourcing ladder creates a new run rather than replacing the old one and comparing two is how anybody answers "why did this find nothing on Tuesday and something on Thursday". `rung_attempt` stores what *every* rung did, including the ones that found nothing and the ones that were skipped, which is what makes an escalation to RFQ explainable to the customer it inconvenienced. A CHECK pins each rung to its position in the ladder, so the database and the code cannot disagree about the order — and the order is the product decision: putting `rfq` first would make JAYA a request board rather than something that tries to solve a Need before publishing it. Migration 0051 widens `rung_attempt.outcome` to separate `lookup-failed` from `unavailable`, because 0050 was carrying two different facts in one word: "no adapter is wired for this rung" is a configuration choice and nobody is paged, while "the adapter was called and it broke" is an outage that is quietly escalating every Need to RFQ for a reason that has nothing to do with supply. Its rollback can fail by design — narrowing the CHECK with `lookup-failed` rows on record would either rewrite an outage into a configuration choice or delete the evidence, and refusing is better than either. Migration 0052 creates the ninth business-module schema, `module_rfq`, with M-09's `rfq`, its append-only `rfq_invitation` and `rfq_event`. The important thing about it is what is absent: there is no column for the customer's words and no free-text column wide enough to hide them in. `item_description` is capped at 500 characters for that reason -- a field long enough to hold a customer message will eventually hold one. `UNIQUE (rfq_id, supplier_account_id)` makes inviting somebody twice impossible rather than merely discouraged, and a CHECK ties `status = awarded` to naming a winning quote in both directions, so an awarded tender can always say who was chosen. Migration 0053 creates the tenth, `module_quotes`, with M-10's `quote` and its outbox. Its defining rule is the `quote_terms_are_immutable` trigger: an offer binds, so the price, quantity, lead time, delivery terms and validity of a submitted quote cannot be changed by any UPDATE, and a supplier who wants a different price withdraws and offers again — which leaves both on the record where a buyer can see that the price moved. The same trigger refuses DELETE and refuses to move an offer that has already ended, so a rejection cannot be taken back. A CHECK ties `kind = 'substitute'` to `substitution_note` in both directions, because an undeclared substitution is how a buyer discovers on delivery day that they did not get what they ordered. **No score, rank or recommendation is stored**: a ranking depends on the weights in force and on what else was offered, and a stale score presented as current is worse than none. Migration 0054 makes an order line's source **either a listing version or an accepted quote**, in both directions by CHECK. 0028 had assumed every line came from a listing, which is right for half the platform and impossible for the other half: a tender exists precisely because no listing answered, so there is no version to pin, and faking one would put a row in M-04 that nobody published and no supplier maintains. `commerce_unit_type_id` is required for a listing line and NULL for a quote line, because a tender is written in whatever unit the buyer used and naming a K-11 type that does not exist would be worse than saying so. `line_kind` separates `goods` from `charges`, because a quote's landed total is deliberately not `quantity * unit_price` — the difference is delivery, duties and handling — and `order_item_line_total_is_product` rightly refuses a line where it is not; so an accepted offer becomes a goods line at the exact product plus a charges line for the rest, and the buyer sees what they are being charged for rather than a unit price quietly inflated to absorb delivery. Its rollback can fail by design: restoring NOT NULL with quote-sourced lines on record would require deleting the record of what a buyer agreed to or inventing a listing for it, and neither is a decision a migration may take on an operator's behalf. Migration 0055 adds M-10's `quote_total_covers_goods`: a landed total is never below the goods it lands, because a total under `quantity * unit_price` says the stated unit price is not the price, and M-11 would have to open the order with a negative charge line or quietly restate a number the supplier committed to. A supplier giving a volume discount states a lower unit price, which is what a discount is. Migration 0056 corrects a defect in K-08 that had been latent since 0004: `UNIQUE (claim_token)` allowed a claim token on exactly one row, so a worker claiming two due deliveries in one call violated it and **batching never worked against PostgreSQL** -- a subscription with a backlog made no progress at all, and the error named a constraint rather than the situation. The in-memory repository allowed batching, so every unit test passed; the failure needed two events due for one subscription at once, which nothing produced until an end-to-end journey accepted two partial offers on one tender. The token guards **completion**, which is per row and never needed global uniqueness; reuse across claims moves to an explicit check inside the same transaction, where the in-memory repository has always done it. Migration 0057 creates the eleventh business-module schema, `module_supplier_directory`, with M-48's `directory_entry`, `supplier_facet`, `supplier_location`, its append-only `directory_event` and its outbox. It is what M-07's `known` and `verified` rungs had nothing to read: until it existed the sourcing ladder could only consult the catalogue and then escalate, so every Need that no listing answered became an RFQ. `UNIQUE (account_id)` makes one party one entry, because a supplier with two directory rows is a supplier who can be invited twice to the same tender and answer once. `supplier_facet_value_is_code` holds a declared category, brand, capability or district to a lower-case code of 2 to 64 characters rather than to the opacity rule the rest of the platform uses for identifiers: a shared vocabulary is meant to be enumerable — a buyer picks `cement` from a list — while an account id is meant not to be. What the rule still refuses is a long run of digits, because a code travels into every invitation the supplier receives and a telephone number pasted into a category field would be published there for good. `supplier_location_one_primary_idx` is a partial unique index giving a supplier exactly one primary branch among their open ones, so "where do we send this" always has an answer and never two; a closed branch is left out of the index rather than deleted, because an order already placed still names it. No verification column exists here and none may: M-02 decides whether a supplier is verified, and a copy of that answer in the directory would be the stale one somebody sources against. |
| Tests | 2362 passing (`npm test`, exit 0) — substrate, boundary enforcement, documentation contract, migration contract, migration runner, seed/fixture contract, and the K-01 Identity, K-02 Authentication, K-03 Accounts, K-04 Permissions, K-05 Configuration, K-06 Policy Engine, K-07 Feature Flags, K-08 Event Infrastructure, K-09 Audit Foundation, K-10 Ledger Foundation, K-12 Conversation Foundation, K-13 AI Gateway, K-14 Notifications and K-15 Search Foundation suites. K-05, K-06, K-07 and K-11 each gained an outbox suite (`configuration-outbox` 2, `policy-engine-outbox` 4, `feature-flags-outbox` 4, `commerce-unit-registry-outbox` 5 cases). K-04 accounts for 106 of them across six suites (`permissions` 19, `permissions-decisions` 23, `permissions-administration` 20, `permissions-repository` 18, `permissions-idempotency` 14, `permissions-concurrency` 12), K-07 for 88 across five (`feature-flags` 17, `feature-flags-evaluation` 20, `feature-flags-repository` 21, `feature-flags-concurrency` 18, `feature-flags-rollout` 12), and K-06 accounts for 81 of them across five suites (`policy-engine-repository` 22, `policy-engine-evaluation` 18, `policy-engine-lifecycle` 15, `policy-engine` 13, `policy-engine-decimal` 13). K-10 accounts for 25 of them across two suites (`ledger-foundation` 15, `ledger-foundation-repository` 10), K-12 accounts for 71 across two (`conversation-foundation` 43, `conversation-foundation-repository` 28), K-13 accounts for 80 across three (`ai-gateway` 47, `ai-gateway-repository` 30, `ai-gateway-mock-provider` 3), and K-14 accounts for 63 across two (`notifications` 36, `notifications-repository` 27), and K-15 accounts for 57 across two (`search-foundation` 30, `search-foundation-repository` 27). M-04 accounts for 52 of them — 40 across the two listing suites and 12 in the parameterised inventory contract suite. A further 210 live-PostgreSQL tests exist; when a PostgreSQL runtime is configured they **pass**, and when none is configured they **skip** with their reason stated |
...
| Deployment | None |
| Monitoring | None |
| Modules implemented | 25 of 62 partially — **K-01 Identity** (FND-004a), **K-02 Authentication** (FND-004c), **K-03 Accounts** (FND-004b), **K-04 Permissions** (FND-004d), **K-05 Configuration** (FND-003a), **K-06 Policy Engine** (FND-005b), **K-07 Feature Flags** (FND-004e), **K-08 Event Infrastructure** (FND-003b), **K-09 Audit Foundation** (FND-003c), **K-10 Ledger Foundation** (FND-005c), **K-12 Conversation Foundation** (FND-005d), **K-13 AI Gateway** (FND-005e), **K-14 Notifications** (FND-005g), **K-15 Search Foundation** (FND-005h), **M-01 Universal Account** (the first business module), **M-02 Capability & Verification**, **M-04 Universal Listing** (listing and inventory halves), **M-09 RFQ / Reverse Marketplace** (tenders, invitations and the privacy boundary), **M-10 Quotes** (supplier offers and the ranking a customer decides on), **M-11 Orders** (including split supplier fulfilment), **M-12 Payments** (provider-neutral orchestration, no live adapter) **M-13 Financial Ledger** (the wallet map and the value router over K-10) and **M-36 User Cockpit** (read-only screens that store nothing) and **M-48 Supplier & Merchant Directory** (the network the sourcing ladder searches) and **M-49 Organisations** (the business as an account of its own, and who may act for it), cores only; 11 of 49 business modules. All 62 registered in the architecture manifest |
...
| Module contracts written | 25 of 62 — [`kernel/identity/CONTRACT.md`](../kernel/identity/CONTRACT.md), [`kernel/authentication/CONTRACT.md`](../kernel/authentication/CONTRACT.md), [`kernel/accounts/CONTRACT.md`](../kernel/accounts/CONTRACT.md), [`kernel/permissions/CONTRACT.md`](../kernel/permissions/CONTRACT.md), [`kernel/configuration/CONTRACT.md`](../kernel/configuration/CONTRACT.md), [`kernel/policy-engine/CONTRACT.md`](../kernel/policy-engine/CONTRACT.md), [`kernel/feature-flags/CONTRACT.md`](../kernel/feature-flags/CONTRACT.md), [`kernel/event-infrastructure/CONTRACT.md`](../kernel/event-infrastructure/CONTRACT.md), [`kernel/audit-foundation/CONTRACT.md`](../kernel/audit-foundation/CONTRACT.md), [`kernel/ledger-foundation/CONTRACT.md`](../kernel/ledger-foundation/CONTRACT.md), [`kernel/conversation-foundation/CONTRACT.md`](../kernel/conversation-foundation/CONTRACT.md), [`kernel/ai-gateway/CONTRACT.md`](../kernel/ai-gateway/CONTRACT.md), [`kernel/notifications/CONTRACT.md`](../kernel/notifications/CONTRACT.md) [`kernel/search-foundation/CONTRACT.md`](../kernel/search-foundation/CONTRACT.md) [`modules/universal-account/CONTRACT.md`](../modules/universal-account/CONTRACT.md) [`modules/capability-verification/CONTRACT.md`](../modules/capability-verification/CONTRACT.md) [`modules/universal-listing/CONTRACT.md`](../modules/universal-listing/CONTRACT.md) [`modules/rfq/CONTRACT.md`](../modules/rfq/CONTRACT.md) [`modules/quotes/CONTRACT.md`](../modules/quotes/CONTRACT.md) [`modules/orders/CONTRACT.md`](../modules/orders/CONTRACT.md) [`modules/payments/CONTRACT.md`](../modules/payments/CONTRACT.md) [`modules/financial-ledger/CONTRACT.md`](../modules/financial-ledger/CONTRACT.md) [`modules/user-cockpit/CONTRACT.md`](../modules/user-cockpit/CONTRACT.md) [`modules/supplier-directory/CONTRACT.md`](../modules/supplier-directory/CONTRACT.md) and [`modules/organisations/CONTRACT.md`](../modules/organisations/CONTRACT.md) |
...
| Verification and evidence | **Foundation only** — ordered verification levels with an append-only level log, one open case per account and purpose enforced by a partial unique index, and evidence held as an **opaque reference to an artefact another system stores, never the artefact**; **no verifier exists**, so M-02 records the level a caller says was reached and checks nothing itself | M-02 |
| Listings | **Foundation only** — M-04. A listing is a stable identity whose terms change by **version, never by edit**, so an order that pinned version 3 still reads what version 3 said; prices are `bigint` minor units with no floating-point column in the schema; media is an opaque handle, never the artefact. The inventory interface is now built with an append-only movement log, a derived snapshot and the six operations (`receive`, `adjust`, `reserve`, `release`, `commit`, `getAvailability`), but **nothing calls it** | M-04 |
| HTTP API | **Foundation only** — `apps/api`. 73 routes over M-03, M-04, M-07, M-09, M-10, M-11, M-12, M-13 and M-36 on `node:http`, with RFC 9457 problem responses that keep each module’s own refusal code, an `Idempotency-Key` requirement on every write, correlation ids on every response, and a request context that is the one place a clock is read or an identifier generated. **No authentication and no authorisation**: K-02 ships no verifier and nothing asks K-04, so every route is open to anyone who can reach the port. Not deployed | apps/api |
| Buyer cockpit | **Foundation only** — M-36. MY MONEY, MY ORDERS and one order in detail, assembled at read time from K-10, M-11, M-12 and M-13. **It stores nothing**: no table, no cached total, no migration — a cached balance on a screen showing somebody their money is a lie with a timestamp, so this module cannot be wrong about money, only slow. MY MONEY is a list of holdings per asset type and **never one total**: 1,500 reward points plus LKR 8,000 is not 9,500 of anything, and each position carries whether the value can be withdrawn, who issued it and how far it divides. **Nothing asks K-04 whether the caller may read this account**, which is the module’s most important gap, and there is no UI | M-36 |
| Businesses and who acts for them | **Foundation only** — M-49. An organisation is a K-03 account of its own, so every commercial record already names it; a membership says one person may act for one business in named roles, and confers nothing at any other. Created atomically with its owner, `pending` until an operator admits it, and unable to be left without an active owner — refused four ways in the service and by a deferred constraint trigger. K-04 now evaluates authority in an account the subject holds a grant in (D-054), which is what lets somebody act for a business without impersonating it; the API declares that with `x-acting-for` and never infers it. **No HTTP routes yet, no invitation UI, and the supplier directory still registers a person account rather than an organisation** | M-49 |
| Supplier and merchant network | **Foundation only** — M-48. The directory the sourcing ladder was built against and did not have: entries, the four facet kinds a supplier declares, branches, status history and a gated search. Registration is deliberately not activation, so a new entry is invisible to the rungs until somebody activates it. **The `known` and `verified` rungs now read it** through an adapter in `apps/` that joins M-48 claims, M-02 verification and M-11 trade history — so a Need the catalogue cannot answer is no longer automatically a tender. Reachable over HTTP: a party registers under their own session, declares what they deal in, opens branches and sets availability, and an OPERATIONS holder admits them with a declared purpose K-04 records. **No UI, no notification, and no supplier reliability figure — M-07 scores every party as having no delivery record** | M-48 |
| Multi-value ledger | **Foundation only** — M-13. The wallet map — which K-10 account holds whose value, in what asset, for what purpose — and the value plan, which pays one obligation from several kinds of value at once and proves the pieces added up exactly. LKR 10,000 as 1,500 reward points + 500 merchant credit + 8,000 on a card is three journal transactions in three units, checked by integer cross-multiplication with no rounding anywhere. Earnings is a purpose, not an asset class. No balance is stored: K-10 derives every one. Nothing calls it — no consumer of M-12’s `payment.captured` hands it the external leg, and no API or UI exists | M-13 |
| Payments | **Foundation only** — M-12. Provider-neutral orchestration of one externally settled payment: authorise, capture, cancel, refund, and webhook receipts that make a provider’s redelivery harmless. A timeout is not a decline, so an indeterminate answer leaves the payment where it was and the caller retries under the same key. No instrument is ever stored — an opaque provider token, held to K-03’s opacity rule in TypeScript and in SQL. Internally issued JAYA value is refused by name: M-13 allocates those legs, and M-12 settles only external rails. **No live gateway adapter ships** (BL-05), nothing calls it, and no consumer reads any of the six events | M-12 |
| Orders | **Foundation only** — M-11. An order is an agreement: each line pins the M-04 listing version it was priced from, and the commercial snapshot captured at placement is what a dispute is judged against. **Split supplier fulfilment** turns a 20-tonne order into a parent with one child order per supplier; partial fulfilment is a quantity derived by summing children, never a status. Nothing calls it: no reservation is taken from M-04, no payment exists (M-12 unbuilt), and no consumer reads any of the seven events | M-11 |
| Tenders (RFQ) | **Foundation only** — M-09. A tender is what happens when M-07's sourcing ladder could not solve a Need any other way, and it carries a **supplier-facing specification, never the customer's words**: a Need is a sentence somebody wrote and a tender goes to strangers. Defended by a carried-key allowlist, a private-text guard, a 500-character description cap, and a schema of named typed columns rather than a blob — there is nowhere to hide a pasted message. No specification travels in an event, so a private tender stays private. One invitation per supplier per tender, append-only, each with a required reason. An award names exactly one winner in both directions. **Nothing calls it**: no route, no notification actually sent, no consumer of any of the five events | M-09 |
| Supplier offers and ranking | **Foundation only** — M-10. An offer binds, so its terms cannot be edited by the service, the adapter or an UPDATE that reaches the table — a supplier changes a price by withdrawing and offering again, which leaves both on the record. Only an invited supplier may quote, and only their own offer may they withdraw. **Ranking is not price**: five weighted factors as integers per mille, weights a buyer can override, a supplier with no record scored 600 rather than 0, unavailable offers shown with their reason, an explanation on every score, and exactly one recommendation the customer may always override. No price travels in an event; the audit record carries it. **Nothing calls it**: no route, and no order is opened from an accepted offer | M-10 |
...
Per v3 §56, completion requires evidence. Below is every evidence claim currently made in this repository: four documentation artefacts from DOC-001 and sixteen delivered engineering tasks, each backed by named commands with recorded exit codes. Seven of those tasks were subsequently corrected after review, and each correction has its own numbered block rather than being folded into the original — an over-claim that is quietly edited away teaches nobody anything. The full set of blocks is §11.1–§11.38. **FND-004d carries three corrections, all of them security corrections**, which is more than any other task in this register and is recorded in §7 as well as here.
...
| M-04B — Universal Listing inventory interface | DELIVERED | Commands + exit codes + parameterised contract test | See §11.38. **Nothing marked COMPLETE.** The M-04 implementation row stays `IN PROGRESS`: the inventory interface now exists with `receive`, `adjust`, `reserve`, `release`, `commit` and `getAvailability`, backed by an append-only movement log and a derived snapshot, but nothing calls it; no API, no UI, no K-11 type check, no K-02 authentication, no K-04 authorisation, no consumer of any inventory event, and nothing applied to a live server. |
...
### 11.38 Evidence — M-04B (Universal Listing inventory interface)

**Task:** M-04B — Universal Listing inventory interface.
**Selected in:** §8 is unchanged; M-04 was already in progress and this slice completes the inventory half of the module.
**Status:** DELIVERED. **Nothing marked COMPLETE.** The M-04 implementation row stays `IN PROGRESS`.

**What M-04B is, in one sentence:** the inventory half of the listing module — availability derived from an append-only movement log, not a mutable counter, so the module remains replaceable per MU-018.

**Delivered:**

| Area | What landed |
|---|---|
| Movement log | `inventory_movement` is append-only: `receive`, `adjust-up`, `adjust-down`, `reserve`, `release`, `commit`; positive quantities only; the kind carries direction like K-10 debit/credit |
| Derived snapshot | `inventory_snapshot` per `(listing_id, version_id)`, written in the same transaction as the movement that changes it; `available = on_hand - reserved` is computed on read |
| Database invariants | CHECK constraints `on_hand >= 0`, `reserved >= 0`, `committed >= 0`, `reserved <= on_hand`; append-only trigger on `inventory_movement` |
| Operations | `receiveInventory`, `adjustInventory`, `reserveInventory`, `releaseInventory`, `commitInventory`, `getAvailability` — listing, version and withdrawal guards reused from slice A |
| Idempotency | Every operation is idempotent by its caller-supplied idempotency key; a replay changes no number and emits no second event |
| Events | Five outbox events — `inventory.received`, `inventory.adjusted`, `inventory.reserved`, `inventory.released`, `inventory.committed` — each with a matching K-09 audit action |
| Contract test | `tests/contracts/inventory.contract.test.ts` parameterised over `InventoryUnderTest`, run against M-04's in-memory implementation |
| Persistence | In-memory reference repository and PostgreSQL adapter, migration 0027 under `module_universal_listing` |

```text
ITEM ID:            M-04B (Universal Listing inventory interface)
MODULE / PHASE:     M-04 Universal Listing / Phase 0, build step B-7
STATUS:             DELIVERED — implementation IN PROGRESS, contract COMPLETE
IMPLEMENTED:        modules/universal-listing/{types,registry,immutable,validate,
                    outbox,repository,service,postgres-repository,index}.ts,
                    CONTRACT.md,
                    db/migrations/0027_create_module_universal_listing_inventory.{up,down}.sql,
                    tests/contracts/inventory.contract.test.ts

COMMANDS:           npm run verify                                    exit 0   tests 1756, pass 1756
                    node --test tests/universal-listing.test.ts tests/universal-listing-repository.test.ts
                                                                      exit 0   tests 40, pass 40
                    node --test tests/contracts/inventory.contract.test.ts
                                                                      exit 0   tests 12, pass 12
                    npm run check:migrations                          exit 0   54 files, 0 violations
                    npm run check:boundaries                          exit 0   208 files, 816 imports, 0 violations
                    npx prettier --check modules/universal-listing db/migrations tests/contracts
                                                                      exit 0
                    npm run test:integration                          exit 0   tests 118, pass 118
                    node docs/tools/validate-doc-links.mjs            exit 0   0 broken
                    npm audit --audit-level=high                      exit 0   0 vulnerabilities
                    git diff --check                                  exit 0

ADVERSARIAL COVER:  Insufficient-stock reservation and adjustment; reserving more than available;
                    interleaved reservations that together exceed availability; release/commit of an
                    unknown or already-settled reservation; snapshot consistency against movement-sum
                    replay; idempotency-key replay; duplicate movement id; duplicate reservation id;
                    malformed movement input; and refusal leaves no movement, snapshot change or
                    outbox entry.

STILL NOT VERIFIED: **Nothing calls the module** — no API, no UI, no consumer of any of the five
                    inventory events, no verification gate against M-02, no K-11 type check, no K-02
                    authentication, no K-04 authorisation, no search indexing into K-15, and nothing
                    applied to a live server beyond the integration suite's isolated `_test` database.
```

```text
ITEM ID:            M-48 / M-49 (the marketplace network, and the businesses in it)
MODULE / PHASE:     M-48 Supplier & Merchant Directory, M-49 Organisations / Phase 0, build steps B-2 and B-7
STATUS:             DELIVERED — implementations IN PROGRESS, contracts COMPLETE
IMPLEMENTED:        modules/supplier-directory/{types,registry,immutable,validate,outbox,repository,
                    service,postgres-repository,index}.ts, CONTRACT.md,
                    modules/organisations/{same nine}.ts, CONTRACT.md,
                    apps/api/supplier-source.ts        (M-48 + M-02 + M-11, joined for M-07's rungs)
                    apps/api/routes/directory.ts       (14 routes)
                    apps/api/routes/participants.ts    (join, sign in, take on a role)
                    apps/api/registrar.ts              (the injected authority behind registration)
                    kernel/permissions/{types,service}.ts   (ORG_* namespace; acting-for, D-054)
                    db/migrations/0057_create_module_supplier_directory_schema.{up,down}.sql
                    db/migrations/0058_create_module_organisations_schema.{up,down}.sql
                    db/migrations/0059_widen_permission_roles_for_organisations.{up,down}.sql

COMMANDS:           npm run verify                                    exit 0   tests 2362, pass 2362
                    node --test tests/supplier-directory.test.ts      exit 0   tests 25, pass 25
                    node --test tests/organisations.test.ts           exit 0   tests 27, pass 27
                    node --test tests/directory-api.test.ts           exit 0   tests 20, pass 20
                    node --test tests/participants-api.test.ts        exit 0   tests 12, pass 12
                    node --test tests/supplier-source.test.ts         exit 0   tests 10, pass 10
                    npm run check:migrations                          exit 0   118 files, 0 violations
                    npm run check:boundaries                          exit 0   340 files, 1458 imports, 0 violations
                    npm run check:fixtures                            exit 0   2 files, 2 datasets, 0 violations
                    npm run test:integration                          exit 0   tests 210, pass 210
                    npm run test:e2e                                  exit 0   tests 8, pass 8
                    git diff --check                                  exit 0

ADVERSARIAL COVER:  **M-48**: registering twice on one account; a supplier admitting themselves; an
                    operator admitting their own entry; a stranger reading another supplier's entry,
                    facets, locations or history; a telephone number as a category; a second primary
                    branch; a withdrawal leaving a live duplicate; an ungated directory query; a
                    pending party appearing in a search; a suspend-then-reactivate cycle colliding on
                    one outbox id; a capacity larger than a double; and a `Date`-rounded instant.
                    **The rungs**: a placed-but-undelivered order counted as prior trade; a past
                    seller who no longer deals in the category; a directory outage reported as an
                    empty market; a Need whose category could not be read producing an ungated query.
                    **M-49**: a second business on one account; a second membership for one person; a
                    MANAGER building a team; an ADMIN conferring, demoting or removing ownership;
                    somebody changing their own roles or suspending themselves; a suspended member
                    still acting; accepting somebody else's invitation; and the last owner leaving,
                    being suspended, being revoked, being demoted, or being deleted with raw SQL.
                    **Registration**: an email, telephone number, name, NIC, address or date of birth
                    at registration; a caller naming their own subject, account or roles; a retry
                    making two people; a wrong password distinguishable from an unknown reference; a
                    role taken on by asking that nobody may take on by asking.
                    **Acting-for**: a member of one business naming a second one; a caller naming a
                    business they hold nothing in; the cross-party exemption leaking to another route.

STILL NOT VERIFIED: **No UI.** No notification when a supplier is invited, admitted or suspended, so
                    an invited supplier still discovers a tender by polling. No HTTP routes for M-49,
                    so organisations can be created only from a test. **The supplier directory still
                    registers the caller's own account rather than an organisation's**, so the two
                    modules do not yet meet. No supplier reliability figure — M-07 scores every party
                    as having no delivery record, which is honest and is not the same as having one.
                    No merchant listing linkage, no member or logistics module, and no transfer of
                    sole ownership other than by making a second owner and leaving.
```
