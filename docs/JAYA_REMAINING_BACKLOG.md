...
| ~~6~~ | — | ~~**Consumer: `payment.captured` → `settleExternalLeg`**~~ | — | S | — | — | — | No | **DONE.** `apps/api/consumers/payment-settlement.ts`, 10 tests. Refuses a capture that does not cover its leg, refuses an undeclared asset mapping, and reports a payment with no plan rather than dead-lettering it. Settles exactly once under at-least-once delivery |
| **7** | M-11 + M-04 | **Reserve inventory when an order line is added** | — | M | Yes | No | No | No | **Started, and it found a defect worse than the missing join: `POST /v1/orders/{id}/items` reads `reservationId` from the request body.** A client can name any string and M-11 records it, so an order line can claim stock nothing is holding — the same shape as the webhook that read `signatureVerified` from its caller. **Blocked on a product decision, see below** |
| **7a** | M-04 | **Whether a listing tracks stock at all** | — | M | Yes | No | No | No | **The decision #7 waits on.** M-04 has no way to say "this listing is a service and holds no stock": availability is derived from movements, so a listing that never received stock is indistinguishable from one that is out of stock. So either *every* order line must reserve — which means JAYA can sell only things with tracked inventory — or M-04 gains an explicit `tracksInventory` fact. The second is right for a platform whose vision includes services (§X), and it needs a migration |
