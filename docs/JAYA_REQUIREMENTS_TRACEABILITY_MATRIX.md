...
- 104 migration files (52 forward, 52 rollback).
...
| G-06 | Partial bid | `INTEGRATION TESTED` 75% | **P0** | `kind = partial`, scored proportionally rather than excluded, and proved end to end: two partials covering one tender exactly, each becoming its own order against its own supplier. **A split award is not modelled** -- an award names exactly one winner, so a split tender is closed rather than awarded (D-048) |
...
|---|---|---|---|---|
| AG-01 | Clean DB migration | `INTEGRATION TESTED` | 95% | 52 forward + 52 rollback, checksum reconciliation, advisory locking, per-migration atomicity. Every module's rollback is proven to leave no trace |
