...
--
-- K-06 Policy Engine outbox (FND-003d).
--
...
-- Infrastructure and K-09 Audit Foundation. Because the outbox lives in K-06's own schema, the
-- module does not need to open K-08 or K-09 transactions to publish atomically.
--
...
CREATE TABLE IF NOT EXISTS kernel_policy_engine.outbox (
...
COMMENT ON TABLE kernel_policy_engine.outbox IS
  'Transactional outbox for policy lifecycle events and audit records, dispatched by a relay.';

-- The relay polls this index for unprocessed rows.
CREATE INDEX IF NOT EXISTS outbox_unprocessed_idx
  ON kernel_policy_engine.outbox (recorded_at, outbox_id)
