-- migration: 0043_add_module_financial_ledger_outbox_relay_columns
-- direction: up
-- owner: module_financial_ledger
--
-- Give module_financial_ledger's outbox what a production relay needs: a time before which a failed row must not
...
ALTER TABLE module_financial_ledger.outbox
...
DROP INDEX IF EXISTS module_financial_ledger.outbox_claimable_idx;

CREATE INDEX outbox_claimable_idx
  ON module_financial_ledger.outbox (next_attempt_at NULLS FIRST, recorded_at, outbox_id)
  WHERE processed_at IS NULL AND dead_lettered_at IS NULL;

COMMENT ON COLUMN module_financial_ledger.outbox.next_attempt_at IS
  'The relay skips this row until this instant. NULL means eligible now.';

COMMENT ON COLUMN module_financial_ledger.outbox.dead_lettered_at IS
