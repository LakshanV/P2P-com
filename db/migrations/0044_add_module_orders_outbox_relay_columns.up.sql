-- migration: 0044_add_module_orders_outbox_relay_columns
-- direction: up
-- owner: module_orders
--
-- Give module_orders's outbox what a production relay needs: a time before which a failed row must not
...
ALTER TABLE module_orders.outbox
...
DROP INDEX IF EXISTS module_orders.outbox_claimable_idx;

CREATE INDEX outbox_claimable_idx
  ON module_orders.outbox (next_attempt_at NULLS FIRST, recorded_at, outbox_id)
  WHERE processed_at IS NULL AND dead_lettered_at IS NULL;

COMMENT ON COLUMN module_orders.outbox.next_attempt_at IS
  'The relay skips this row until this instant. NULL means eligible now.';

COMMENT ON COLUMN module_orders.outbox.dead_lettered_at IS
