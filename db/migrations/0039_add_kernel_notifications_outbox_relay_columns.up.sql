-- migration: 0039_add_kernel_notifications_outbox_relay_columns
-- direction: up
-- owner: kernel_notifications
--
-- Give kernel_notifications's outbox what a production relay needs: a time before which a failed row must not
...
ALTER TABLE kernel_notifications.outbox
...
DROP INDEX IF EXISTS kernel_notifications.outbox_claimable_idx;

CREATE INDEX outbox_claimable_idx
  ON kernel_notifications.outbox (next_attempt_at NULLS FIRST, recorded_at, outbox_id)
  WHERE processed_at IS NULL AND dead_lettered_at IS NULL;

COMMENT ON COLUMN kernel_notifications.outbox.next_attempt_at IS
  'The relay skips this row until this instant. NULL means eligible now.';

COMMENT ON COLUMN kernel_notifications.outbox.dead_lettered_at IS
