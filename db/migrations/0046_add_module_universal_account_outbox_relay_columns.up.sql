...
--
-- Give module_universal_account's outbox what a production relay needs: a time before which a failed row must not
...
CREATE INDEX outbox_claimable_idx
  ON module_universal_account.outbox (next_attempt_at NULLS FIRST, recorded_at, outbox_id)
  WHERE processed_at IS NULL AND dead_lettered_at IS NULL;

COMMENT ON COLUMN module_universal_account.outbox.next_attempt_at IS
  'The relay skips this row until this instant. NULL means eligible now.';

COMMENT ON COLUMN module_universal_account.outbox.dead_lettered_at IS
