-- migration: 0045_add_module_payments_outbox_relay_columns
-- direction: up
-- owner: module_payments
--
-- Give module_payments's outbox what a production relay needs: a time before which a failed row must not
...
ALTER TABLE module_payments.outbox
...
DROP INDEX IF EXISTS module_payments.outbox_claimable_idx;

CREATE INDEX outbox_claimable_idx
  ON module_payments.outbox (next_attempt_at NULLS FIRST, recorded_at, outbox_id)
  WHERE processed_at IS NULL AND dead_lettered_at IS NULL;

COMMENT ON COLUMN module_payments.outbox.next_attempt_at IS
  'The relay skips this row until this instant. NULL means eligible now.';

COMMENT ON COLUMN module_payments.outbox.dead_lettered_at IS
