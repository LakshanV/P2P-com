-- migration: 0040_add_kernel_policy_engine_outbox_relay_columns
-- direction: up
-- owner: kernel_policy_engine
--
-- Give kernel_policy_engine's outbox what a production relay needs: a time before which a failed row must not
...
ALTER TABLE kernel_policy_engine.outbox
...
DROP INDEX IF EXISTS kernel_policy_engine.outbox_claimable_idx;

CREATE INDEX outbox_claimable_idx
  ON kernel_policy_engine.outbox (next_attempt_at NULLS FIRST, recorded_at, outbox_id)
  WHERE processed_at IS NULL AND dead_lettered_at IS NULL;

COMMENT ON COLUMN kernel_policy_engine.outbox.next_attempt_at IS
  'The relay skips this row until this instant. NULL means eligible now.';

COMMENT ON COLUMN kernel_policy_engine.outbox.dead_lettered_at IS
