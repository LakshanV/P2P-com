-- migration: 0036_add_kernel_conversation_foundation_outbox_relay_columns
-- direction: up
-- owner: kernel_conversation_foundation
--
-- Give kernel_conversation_foundation's outbox what a production relay needs: a time before which a failed row must not
...
ALTER TABLE kernel_conversation_foundation.outbox
...
DROP INDEX IF EXISTS kernel_conversation_foundation.outbox_claimable_idx;

CREATE INDEX outbox_claimable_idx
  ON kernel_conversation_foundation.outbox (next_attempt_at NULLS FIRST, recorded_at, outbox_id)
  WHERE processed_at IS NULL AND dead_lettered_at IS NULL;

COMMENT ON COLUMN kernel_conversation_foundation.outbox.next_attempt_at IS
  'The relay skips this row until this instant. NULL means eligible now.';

COMMENT ON COLUMN kernel_conversation_foundation.outbox.dead_lettered_at IS
