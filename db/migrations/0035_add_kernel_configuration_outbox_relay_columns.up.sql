-- migration: 0035_add_kernel_configuration_outbox_relay_columns
-- direction: up
-- owner: kernel_configuration
--
-- Give kernel_configuration's outbox what a production relay needs: a time before which a failed row must not
...
ALTER TABLE kernel_configuration.outbox
...
DROP INDEX IF EXISTS kernel_configuration.outbox_claimable_idx;

CREATE INDEX outbox_claimable_idx
  ON kernel_configuration.outbox (next_attempt_at NULLS FIRST, recorded_at, outbox_id)
  WHERE processed_at IS NULL AND dead_lettered_at IS NULL;

COMMENT ON COLUMN kernel_configuration.outbox.next_attempt_at IS
  'The relay skips this row until this instant. NULL means eligible now.';

COMMENT ON COLUMN kernel_configuration.outbox.dead_lettered_at IS
