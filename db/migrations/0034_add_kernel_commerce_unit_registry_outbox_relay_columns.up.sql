-- migration: 0034_add_kernel_commerce_unit_registry_outbox_relay_columns
-- direction: up
-- owner: kernel_commerce_unit_registry
--
-- Give kernel_commerce_unit_registry's outbox what a production relay needs: a time before which a failed row must not
...
ALTER TABLE kernel_commerce_unit_registry.outbox
...
DROP INDEX IF EXISTS kernel_commerce_unit_registry.outbox_claimable_idx;

CREATE INDEX outbox_claimable_idx
  ON kernel_commerce_unit_registry.outbox (next_attempt_at NULLS FIRST, recorded_at, outbox_id)
  WHERE processed_at IS NULL AND dead_lettered_at IS NULL;

COMMENT ON COLUMN kernel_commerce_unit_registry.outbox.next_attempt_at IS
  'The relay skips this row until this instant. NULL means eligible now.';

COMMENT ON COLUMN kernel_commerce_unit_registry.outbox.dead_lettered_at IS
