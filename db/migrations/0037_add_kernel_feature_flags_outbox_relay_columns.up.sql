...
--
-- Give kernel_feature_flags's outbox what a production relay needs: a time before which a failed row must not
...
CREATE INDEX outbox_claimable_idx
  ON kernel_feature_flags.outbox (next_attempt_at NULLS FIRST, recorded_at, outbox_id)
  WHERE processed_at IS NULL AND dead_lettered_at IS NULL;

COMMENT ON COLUMN kernel_feature_flags.outbox.next_attempt_at IS
  'The relay skips this row until this instant. NULL means eligible now.';

COMMENT ON COLUMN kernel_feature_flags.outbox.dead_lettered_at IS
