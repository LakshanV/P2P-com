...
| Refusal | Prevents |
|---|---|
| `malformed-manifest` | a format change being read as if it were the old format, and every runtime shape TypeScript cannot enforce: a non-string dependency, an identity or JSON column that is not lower_snake_case, a duplicate column declaration, a column declared as both identity and JSON, a row that is an array or a string, a table that is not an object |
...
| `fingerprint-mismatch` | a row carrying a `payload_fingerprint` that is not the fingerprint of its own `payload`, and — for `kernel_event_infrastructure.event` — a row carrying **no** fingerprint, no payload, or a payload that is not an object. Recomputed with the same canonical SHA-256 K-08 uses, before any database access |
...
### Column names are the one place caller input reaches SQL as text

Values are parameterised; identifiers cannot be. `INSERT INTO t (a, b) … ON CONFLICT (a) DO NOTHING`
and `DELETE FROM t WHERE a = $1` both interpolate column names directly, so every name in
`identity` and `jsonColumns` must match `^[a-z][a-z0-9]*(_[a-z0-9]+)*# db/fixtures — seed data
...
| `fingerprint-mismatch` | a row carrying a `payload_fingerprint` that is not the fingerprint of its own `payload`, and — for `kernel_event_infrastructure.event` — a row carrying **no** fingerprint, no payload, or a payload that is not an object. Recomputed with the same canonical SHA-256 K-08 uses, before any database access |
 ...
 — a quote, a space, a
parenthesis or a semicolon is refused before any statement is built. The same rule applies to the
table name, which must be `schema.table` in the same character set.

This matters most for a **programmatic** caller. A fixture read from a file has been through
`JSON.parse` and the file is in review; a caller that built a manifest in code, cast an `unknown`,
or deserialised one off a wire has been through neither, and TypeScript enforces nothing at runtime.
`tests/seed-adversarial.test.ts` casts past the type system deliberately and proves each shape is
refused with no connection opened.

### Event rows must carry their evidence

Every row targeting `kernel_event_infrastructure.event` must have an object `payload` **and** a
correctly formatted `payload_fingerprint` of it. Checking only the fingerprints that happen to be
present would let a row opt out by omitting the field — and the row that omits it is exactly the one
nobody computed a fingerprint for. The table is named in `FINGERPRINTED_TABLES`; a test asserts that
name still matches K-08's own `EVENT_TABLE`, since `platform/` may not import upward to ask.
